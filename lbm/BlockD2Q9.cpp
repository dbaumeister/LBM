...
void BlockD2Q9::collide() {
    for (int iX = 0; iX < dimX; ++iX) {
        for (int iY = 0; iY < dimY; ++iY) {
            ...
            fTmp[i + 2] =
                    (1 - omega) * f[i + 2] + omega * computeLocalEquilibrium(1. / 36., 1, -1, rho, &u[0], uSquare);
            fTmp[i + 3 * N] =
                    (1 - omega) * f[i + 3 * N] + omega * computeLocalEquilibrium(1. / 9., -1, 0, rho, &u[0], uSquare);
            fTmp[i + 3 * N + 1] = (1 - omega) * f[i + 3 * N + 1] +
                                  omega * computeLocalEquilibrium(4. / 9., 0, 0, rho, &u[0], uSquare);
            fTmp[i + 3 * N + 2] = (1 - omega) * f[i + 3 * N + 2] +
                                  omega * computeLocalEquilibrium(1. / 9., 1, 0, rho, &u[0], uSquare);
            fTmp[i + 6 * N] =
                    (1 - omega) * f[i + 6 * N] + omega * computeLocalEquilibrium(1. / 36., -1, 1, rho, &u[0], uSquare);
            fTmp[i + 6 * N + 1] = (1 - omega) * f[i + 6 * N + 1] +
                                  omega * computeLocalEquilibrium(1. / 9., 0, 1, rho, &u[0], uSquare);
            fTmp[i + 6 * N + 2] = (1 - omega) * f[i + 6 * N + 2] +
